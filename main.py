...
def main():
    ...
    ##载入图像分类数据集
    train_dataset,val_dataset,test_dataset,class_num = data_load.data_load('slipt_dataset')
    ##创建映射索引
    ...
    optimizer = optim.Adam(vgg16.classifier[6].parameters(),lr=0.001)##只训练最后一层
    ##训练设置
    ...
    EPOCHS = 20
    ##学习率下降策略
    lr_scheduler_1 = lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.5)
    ##训练前设置
    epoch = 0
    batch_idx = 0
    ...
    ##训练可视化
    wandb.init(project='VGG++', name='25a_2020(base)')
    print(' ')
    ##循环训练
    for epoch in range(1, EPOCHS + 1):
        ...
        if log_val['val_accuracy'] > best_val_accuracy:
            # 删除旧的最佳模型文件(如有)
            old_best_checkpoint_path = 'torch/hub/checkpoints/best-25a-2020-{:.4f}.pth'.format(best_val_accuracy)
            if os.path.exists(old_best_checkpoint_path):
                os.remove(old_best_checkpoint_path)
            # 保存新的最佳模型文件
            best_val_accuracy = log_val['val_accuracy']
            new_best_checkpoint_path = 'torch/hub/checkpoints/best-25a-2020-{:.4f}.pth'.format(log_val['val_accuracy'])
            torch.save(vgg16, new_best_checkpoint_path)
            print('保存新的最佳模型', 'checkpoint/best-25a-2020-{:.4f}.pth'.format(best_val_accuracy))


    tab_train_log.to_csv('log/训练日志25a-2020.csv', index=False)
    tab_val_log.to_csv('log/验证日志25a-2020.csv', index=False)
    # 载入最佳模型作为当前模型
    model = torch.load('checkpoint/best-25a-2020-{:.4f}.pth'.format(best_val_accuracy))
